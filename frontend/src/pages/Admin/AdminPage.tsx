...
import { createBL, createProduct, createShip, createUser, getProducts, getShips, getUsers, resetUserPassword, updateShipStatus, updateUserRole, updateUserStatus } from '../../services/adminService'
...
function roleName(role: string | number) {
  ...
  if (role === 4 || role === 'Management') return 'Gerencia'
  return 'User'
}
...
function roleValue(role: string | number) {
  ...
  if (role === 4 || role === 'Management') return 4
  return 3
}

function isShipActive(ship: Ship) {
  return ship.status === 1 || ship.status === 'Active'
}
...
export function AdminPage() {
  ...
  async function load() {
    try {
      const [shipData, productData, userData] = await Promise.all([getShips(), getProducts(), getUsers()])
      setShips(shipData)
  ...
  async function changeShipStatus(ship: Ship) {
    setError(''); setMessage('')
    const nextStatus = !isShipActive(ship)
    try {
      await updateShipStatus(ship.id, nextStatus)
      setMessage(`${ship.name} ${nextStatus ? 'activada' : 'desactivada'} correctamente.`)
      await load()
    } catch (x) { setError(errorMessage(x)) }
  }
  ...
  const activeShips = ships.filter(isShipActive)
      ...
      <div className="admin-layout">
        <aside className="admin-sidebar" aria-label="Menú de configuración">
          ...
          <nav className="admin-nav">
            {menuItems.map((item) => (
              <button type="button" key={item.id} className={`admin-nav-item${section === item.id ? ' is-active' : ''}`} onClick={() => setSection(item.id)}>
          ...
          <div className="admin-sidebar-user"><span>Sesión</span><strong>{user.fullName}</strong><small>Administrador</small></div>
        </aside>
        ...
        <div className="admin-content">
          {section === 'overview' ? (
            <section className="admin-panel">
              ...
              <div className="admin-summary-grid">
                <button type="button" className="admin-summary-card" onClick={() => setSection('ships')}><span className="admin-summary-icon">NV</span><div><strong>{activeShips.length}</strong><span>Naves activas</span><small>{ships.length} registradas</small></div></button>
              ...
              <div className="admin-info-card"><span className="admin-info-badge">Administración</span><div><h3>Configuración centralizada</h3><p>Solo las naves activas estarán disponibles para los Yard al iniciar o cambiar una operación.</p></div></div>
            </section>
          ) : null}

          {section === 'ships' ? (
            <section className="admin-panel">
              <div className="admin-panel-heading"><div><span className="eyebrow">Catálogo</span><h2>Naves</h2><p>Crea naves y controla cuáles estarán disponibles para la operación.</p></div><span className="admin-count-badge">{activeShips.length} activas · {ships.length} registradas</span></div>
              <div className="admin-two-column">
                ...
                <div className="admin-list-card"><h3>Naves registradas</h3>{ships.length === 0 ? <p className="admin-empty">No hay naves registradas.</p> : <div className="admin-simple-list">{ships.map((ship) => { const active = isShipActive(ship); return <div className="admin-simple-row" key={ship.id}><span className={`status-dot${active ? '' : ' inactive'}`} /><strong>{ship.name}</strong><span className={`admin-state ${active ? 'active' : 'inactive'}`}>{active ? 'Activa' : 'Inactiva'}</span><button type="button" className="secondary-action" onClick={() => void changeShipStatus(ship)}>{active ? 'Desactivar' : 'Activar'}</button></div> })}</div>}</div>
              </div>
            </section>
          ) : null}
          ...
          {section === 'bls' ? (
            <section className="admin-panel">
              ...
              <div className="admin-form-card admin-form-wide"><h3>Nuevo BL</h3><form className="admin-bl-form" onSubmit={submitBL}><label>Código<input value={bl.code} onChange={(e) => setBL((current) => ({ ...current, code: e.target.value }))} placeholder="Ej. BL-001" required /></label><label>Cantidad declarada (Big Bags)<input type="number" min="1" step="1" inputMode="numeric" value={bl.totalQuantity} onChange={(e) => setBL((current) => ({ ...current, totalQuantity: e.target.value }))} required /></label><label>Nave<select value={bl.shipId} onChange={(e) => changeBLShip(e.target.value)} required>{ships.map((ship) => <option key={ship.id} value={ship.id}>{ship.name}{isShipActive(ship) ? '' : ' · Inactiva'}</option>)}</select></label><label>Producto<select value={bl.productId} onChange={(e) => setBL((current) => ({ ...current, productId: e.target.value }))} required>{activeProducts.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select></label><button type="submit">Crear BL</button></form></div>
              <div className="admin-list-card admin-bl-list"><div className="admin-list-heading"><h3>BL registrados</h3><label>Nave<select value={bl.shipId} onChange={(e) => changeBLShip(e.target.value)}>{ships.map((ship) => <option key={ship.id} value={ship.id}>{ship.name}{isShipActive(ship) ? '' : ' · Inactiva'}</option>)}</select></label></div>{bls.length === 0 ? <p className="admin-empty">No hay BL registrados para esta nave.</p> : <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Código</th><th>Producto</th><th>Big Bags declarados</th><th>Estado</th></tr></thead><tbody>{bls.map((item) => <tr key={item.id}><td><strong>{item.code}</strong></td><td>{item.productName}</td><td>{item.totalQuantity.toLocaleString('es-PE')}</td><td><span className={`admin-state ${item.isActive ? 'active' : 'inactive'}`}>{item.isActive ? 'Activo' : 'Inactivo'}</span></td></tr>)}</tbody></table></div>}</div>
            </section>
          ) : null}
          ...
          {section === 'users' ? (
            <section className="admin-panel">
              ...
              <div className="admin-form-card admin-form-wide"><h3>Nuevo usuario</h3><form className="admin-user-form" onSubmit={submitUser}><label>UID<input value={newUser.uid} onChange={(e) => setNewUser((current) => ({ ...current, uid: e.target.value }))} maxLength={50} autoComplete="off" required /></label><label>Nombre<input value={newUser.firstName} onChange={(e) => setNewUser((current) => ({ ...current, firstName: e.target.value }))} required /></label><label>Apellido<input value={newUser.lastName} onChange={(e) => setNewUser((current) => ({ ...current, lastName: e.target.value }))} required /></label><label>Contraseña temporal<input type="password" value={newUser.password} onChange={(e) => setNewUser((current) => ({ ...current, password: e.target.value }))} minLength={8} autoComplete="new-password" required /></label><label>Rol<select value={newUser.role} onChange={(e) => setNewUser((current) => ({ ...current, role: Number(e.target.value) }))}><option value={3}>User</option><option value={4}>Gerencia</option><option value={2}>Yard</option><option value={1}>Administrator</option></select></label><button type="submit">Crear usuario</button></form></div>
              <div className="admin-users-list">{users.map((item) => <article className="admin-user-card" key={item.id}><div className="admin-user-main"><div className="admin-user-avatar">{item.fullName?.trim().charAt(0).toUpperCase() || item.uid.charAt(0).toUpperCase()}</div><div><strong>{item.fullName}</strong><span>{item.uid}</span><small>{roleName(item.role)} · {item.isActive ? 'Activo' : 'Inactivo'}{item.uid === user?.uid ? ' · sesión actual' : ''}</small></div></div><div className="admin-user-actions"><select aria-label={`Rol de ${item.uid}`} value={roleValue(item.role)} disabled={item.uid === user?.uid} onChange={(e) => void changeRole(item, Number(e.target.value))}><option value={1}>Administrator</option><option value={2}>Yard</option><option value={3}>User</option><option value={4}>Gerencia</option></select><button type="button" className="secondary-action" disabled={item.uid === user?.uid} onClick={() => void changeStatus(item)}>{item.isActive ? 'Desactivar' : 'Activar'}</button><button type="button" className="secondary-action" onClick={() => { setResetUserId(resetUserId === item.id ? '' : item.id); setResetPassword(''); setError(''); setMessage('') }}>{resetUserId === item.id ? 'Cancelar' : 'Restablecer contraseña'}</button></div>{resetUserId === item.id ? <form className="admin-reset-form" onSubmit={(e) => void submitPasswordReset(e, item)}><label>Nueva contraseña temporal<input type="password" value={resetPassword} onChange={(e) => setResetPassword(e.target.value)} minLength={8} autoComplete="new-password" required /></label><button type="submit">Guardar nueva contraseña</button></form> : null}</article>)}</div>
            </section>
          ) : null}
        </div>
      </div>
