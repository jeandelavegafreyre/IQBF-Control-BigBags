        ...
        <div className="admin-content">
          ...
          {section === 'users' ? (
            <section className="admin-panel">
              ...
              <div className="admin-users-list">{users.map((item) => <article className="admin-user-card" key={item.id}><div className="admin-user-main"><div className="admin-user-avatar">{item.fullName?.trim().charAt(0).toUpperCase() || item.uid.charAt(0).toUpperCase()}</div><div><strong>{item.fullName}</strong><span>{item.uid}</span><small>{roleName(item.role)} · {item.isActive ? 'Activo' : 'Inactivo'}{item.uid === user?.uid ? ' · sesión actual' : ''}</small></div></div><div className="admin-user-actions"><select aria-label={`Rol de ${item.uid}`} value={roleValue(item.role)} disabled={item.uid === user?.uid} onChange={(e) => void changeRole(item, Number(e.target.value))}><option value={1}>Administrator</option><option value={2}>Yard</option><option value={3}>User</option></select><button type="button" className="secondary-action" disabled={item.uid === user?.uid} onClick={() => void changeStatus(item)}>{item.isActive ? 'Desactivar' : 'Activar'}</button><button type="button" className="secondary-action" onClick={() => { setResetUserId(resetUserId === item.id ? '' : item.id); setResetPassword(''); setError(''); setMessage('') }}>{resetUserId === item.id ? 'Cancelar' : 'Restablecer contraseña'}</button></div>{resetUserId === item.id ? <form className="admin-reset-form" onSubmit={(e) => void submitPasswordReset(e, item)}><label>Nueva contraseña temporal<input type="password" value={resetPassword} onChange={(e) => setResetPassword(e.target.value)} minLength={8} autoComplete="new-password" required /></label><button type="submit">Guardar nueva contraseña</button></form> : null}</article>)}</div>
            </section>
          ) : null}
        </div>
      </div>
