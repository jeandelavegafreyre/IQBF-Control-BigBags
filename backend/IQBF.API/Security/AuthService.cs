...
public class AuthService : IAuthService
{
    ...
    public async Task<AuthResponse> LoginAsync(LoginRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var user = await _db.Users.FirstOrDefaultAsync(x => x.UID == uid, cancellationToken)
            ?? throw new UnauthorizedAccessException("UID o contraseña incorrectos.");
        if (!user.IsActive) throw new UnauthorizedAccessException("El usuario está inactivo.");
        var result = _passwordHasher.VerifyHashedPassword(user, user.PasswordHash, request.Password);
        if (result == PasswordVerificationResult.Failed) throw new UnauthorizedAccessException("UID o contraseña incorrectos.");
    ...
    {
        var uid = Normalize(request.UID); var firstName = Normalize(request.FirstName); var lastName = Normalize(request.LastName);
        if (string.IsNullOrWhiteSpace(uid)) throw new ArgumentException("UID es obligatorio.");
        if (string.IsNullOrWhiteSpace(firstName)) throw new ArgumentException("Nombre es obligatorio.");
        if (string.IsNullOrWhiteSpace(lastName)) throw new ArgumentException("Apellido es obligatorio.");
    ...
    public async Task ResetPasswordAsync(Guid userId, ResetUserPasswordRequest request, string actorUid, CancellationToken cancellationToken = default)
    {
    ...
    private AuthResponse CreateResponse(User user)
    {
        ...
        var issuer = _configuration["Jwt:Issuer"]; var audience = _configuration["Jwt:Audience"];
        var expirationMinutes = int.TryParse(_configuration["Jwt:ExpirationMinutes"], out var parsed) ? parsed : 480;
        ...
        var tokenValue = new System.IdentityModel.Tokens.Jwt.JwtSecurityTokenHandler().WriteToken(token);
        return new AuthResponse(tokenValue, expiresAt, user.Id, user.UID, user.FullName, user.Role);
    }
